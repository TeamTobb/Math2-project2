...
- \\
({c^2}{t^2_1} - 2{t_1}d{c^2} + {c^2}{d^2}) + ({c^2}{t^2_2} - 2{t_2}d{c^2} + {c^2}{d^2}) \\
= \\
0 \\ \nonumber
\end{multline} 
...
Merk at likning nr. 3 og 4 også skulle trekkes fra den første fra ligningssettet gitt i (\ref{Ex1:system}). De to nye likningene vil gi de samme resultatene. De eneste forskjellene vil være koordinatene til satelittene. De tre likningene blir:
\begin{align}
...
Videre bruker vi loven som sier at determinanten til matriser med repeterende kolonner er 0 \cite{Determinants}. Da står vi igjen med: 
...
\begin{multline}\label{eq:d_expanded}
    ...
    +\\
    z_1^2+2z_1z_2+z_2^2-2Cz_1-2Cz_2+C^2-c^2{t_1}^2-c^22{t_1}d-c^2d^2 \\
    = 0\\
\end{multline}
